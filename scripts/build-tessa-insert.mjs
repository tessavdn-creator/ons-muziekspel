// Inlegkaart voor Tessa's doosje: 122 x 52 mm, past in de opening en ligt
...
const output = process.argv[2] || path.join(root, '..', '1 PRINTEN', 'Tessa', 'Cadeau en uitnodiging', 'TRACKBACK-Tessa-inlegkaart-doosje.pdf')

const key = (await readFile(path.join(root, '.private/gifts/tessa.key'), 'utf8')).trim()
const giftUrl = `${baseUrl}/#gift=g-t7k2m9wq.${key}`
const qr = await QRCode.toDataURL(giftUrl, { width: 900, margin: 1, errorCorrectionLevel: 'H', color: { dark: '#2b0a3d', light: '#ffffff' } })
...
const voorkant = maakKaart('Alles Door Elkaar', metKaarten, 'Bij Tijdlijn hoef je niets in te typen. Alleen scannen, luisteren en de kaart neerleggen.')
...
const html = `<!doctype html><html lang="nl"><head><meta charset="utf-8"><title>TRACKBACK inlegkaart Tessa</title><style>
...
.kaart{width:122mm;height:52mm;overflow:hidden;padding:4.5mm 5.5mm;color:#2b0a3d;background:#fff;border:.4mm solid #2b0a3d;display:grid;grid-template-columns:30mm 1fr;gap:5mm;align-items:center;break-inside:avoid;print-color-adjust:exact;-webkit-print-color-adjust:exact}
.qr{width:30mm;height:30mm;background:#fff}
.qr img{display:block;width:100%;height:100%}
.merk{font-family:"Arial Black",Arial,sans-serif;font-size:7.5pt;letter-spacing:.14em;color:#ff4fa3}
.tekst h1{margin:.8mm 0 1.6mm;font-family:"Arial Black",Arial,sans-serif;font-size:11.5pt;line-height:1;color:#2b0a3d}
.tekst ol{margin:0;padding-left:4mm;font-size:6pt;line-height:1.4;color:#3b1a4a}
.tekst li{margin-bottom:.35mm}
.tekst li::marker{color:#ff4fa3;font-weight:900}
.tekst small{display:block;margin-top:1.6mm;padding-top:1.2mm;border-top:.3mm solid #ff4fa3;font-size:5.2pt;color:#6d5578}
</style></head><body>
