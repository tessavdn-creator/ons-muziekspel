...
const outputRoot = path.resolve(process.env.TRACKBACK_PRINTABLE_OUTPUT || path.join(root, '..', 'NIEUW COMPACT 60mm', 'KLAAR OM TE PRINTEN'))
...
const LAYOUTS = {
  60: {
    ...
    cornerMarks: true, guide: { vInset: 2.5, vLength: 4, hInset: 2.5, hLength: 4 },
    ruler: false,
    extraCss: '',
  },
  50: {
    card: 50, columns: 4, rows: 5, gap: 0, qr: 31, qrPad: 1.2,
    cornerMarks: false, guide: { vInset: 17, vLength: 6, hInset: 0.8, hLength: 3.6 },
    ...
    extraCss: '.front{padding:2.2mm}.card-brand{top:2mm;left:2.4mm;font-size:4.8pt}'
      + '.edition{top:2.3mm;left:21mm;right:2.4mm;font-size:3.6pt}'
      + '.front>strong{margin-top:1.4mm;font-size:4.6pt}'
      + '.front>span{left:2.4mm;bottom:2.2mm;font-size:4.1pt}'
      + '.back{padding:3mm}.back-brand{top:2.2mm;left:3mm;right:3mm;font-size:4.3pt}'
      + '.year{font-size:16pt;padding:.7mm 2mm;border-width:.45mm;margin:1.6mm 0 2mm}'
      + '.answer{max-width:43mm}.answer strong{font-size:8.4pt}'
      ...
      + '.edition-code{right:2mm;bottom:2mm;max-width:25mm;font-size:3.3pt}',
  },
}
...
const printRuler = () => (layout.ruler
  ? `<div class="ruler" aria-hidden="true">${Array.from({ length: 11 }, (_, index) => `<i style="--mm:${index * 10}mm"></i>`).join('')}<span>100 mm · print op ware grootte, niet passend maken</span></div>`
  : '')
...
async function cardsHtml(edition, artistPortraits) {
  ...
  const tracks = edition.tracks.map(normalizeTrack)
  ...
  for (const [sheetIndex, sheet] of sheets.entries()) {
    ...
    const fronts = await Promise.all(padded.map(async (track, index) => {
      ...
      return cardSlot(`<div class="card front" style="--accent:${accent}"><div class="card-brand brand">TRACKBACK</div><div class="edition">${escapeHtml(edition.name)}</div>${lineupMarkup}<div class="qr-shell"><img src="${qr}" alt=""></div><strong>SCAN · LUISTER · PLAATS</strong><span>KAART ${String(sheetIndex * cardsPerSheet + index + 1).padStart(3, '0')}</span><b class="edition-code">${escapeHtml(editionLabel)}</b></div>`, true)
    }))
    const backs = mirrorRows(padded, layout.columns).map(track => {
      if (!track) return cardSlot('<div class="card blank"></div>')
      const accent = GENRE_COLOR[track.genre] || primary
  ...
  const rulerCss = layout.ruler
    ? '.ruler{position:absolute;left:50%;top:5mm;width:100mm;height:9mm;margin-left:-50mm;z-index:40;pointer-events:none}'
      + '.ruler:before{content:"";position:absolute;left:0;top:0;width:100mm;height:.35mm;background:#000}'
      + '.ruler i{position:absolute;left:var(--mm);top:0;width:.35mm;height:2.4mm;background:#000;transform:translateX(-.175mm)}'
      + '.ruler i:first-child,.ruler i:last-child{height:4mm}'
      + '.ruler span{position:absolute;left:0;top:4.6mm;width:100mm;text-align:center;font:800 5pt/1 Arial,sans-serif;letter-spacing:.04em;color:#110e18}'
    : ''
...
async function main() {
  ...
  try {
    for (const edition of editions) {
      const folder = path.join(outputRoot, edition.owner, EDITION_ORDER[edition.id] || edition.name)
